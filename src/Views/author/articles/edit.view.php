        ...
        <div class="mx-auto max-w-5xl flex flex-col gap-8">
            <div class="flex flex-col gap-1">
                <div class="flex items-center justify-between">
                    <h2 class="text-2xl font-bold text-text-main dark:text-white">Edit Article</h2>
                </div>
                <p class="text-sm text-text-secondary dark:text-gray-400">Update your content, change
                    categories, or modify settings.</p>
            </div>
            <form action="<?= route('author.articles.update') ?>" method="post" enctype="multipart/form-data"
                class="flex flex-col gap-6 rounded-2xl bg-background-card dark:bg-[#1a261d] p-8 shadow-soft">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input name="title"
                        ...
                        id="article-title" placeholder="Enter an engaging title..." type="text" value="<?= $article->title ?>" />
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="flex flex-col gap-2 md:col-span-2">
                        ...
                        <select name="categories[]"
                            class="flex w-full min-h-[50px] flex-wrap items-center gap-2 rounded-xl border border-[#cfdfd0] bg-white dark:bg-[#253528] dark:border-[#2a382d] p-2 pr-10 shadow-sm focus-within:border-primary focus-within:ring-1 focus-within:ring-primary"
                            ...
                            <?php foreach ($categories as $category): ?>
                                ...
                                ?>
                                <option <?php if ($isSelected): ?>selected<?php endif; ?> value="<?= $category->id ?>"><?= $category->name ?></option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="flex flex-col gap-2 md:col-span-1">
                        ...
                        <div
                            ...
                            <input name="cover" class="absolute inset-0 opacity-0 cursor-pointer" type="file" value="<?= $article->cover ?>" />
                        </div>
